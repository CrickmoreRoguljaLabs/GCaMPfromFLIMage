...
% Version 7 allows you to choose when the baseline starts

% Version 7 Mac allows the code to run on MacOS
    % tested that a single csv with one ROI works in fastmode
    % tested that multiple csvs each with one ROI works in fastmode
    % tested that averaging works
    % tested that baseline F value specification works

    % not running in fastmode currently throws an error/not updated and
    % therefore not available yet
    % mutliple ROI function not tested/updated

%% Ask the user if they are running multi-ROI mode
% this feature is not available yet on MacOS
...
%% Ask the user what frame they want to be the start of the baseline
disp('--------------------------------------------------------------------')
disp('Note for Lauren: enter 1095 for 10s before 150s for DA titr')
start = input('At what frame do you want the baseline calculation to start? ');
...
% fastmode is the default on MacOS
...
if multi_ROI == 1
   ...
   singleSheet = 0;
    ...
    if singleSheet == 1 % Bug fix for if there's only one file name
        ...
        % lengths
        % Our data is in the column/row labeled sumIntensity_bg-ROI1-ch1
        sheetData = rows2vars(sheetData); % We need to transpose it so the data is in column form
        tempNew = sheetData.Var18; % Because we took the transpose it is now Var18 that has the data
        tempNew = cell2mat(tempNew(2:end)); % changes the data to array format and removes the column heading
        tempStore{1} = tempNew;
        ...
        for x = 1:length(fileNames) % For each spreadsheet
            cd(pathNames) % Go to where your spreadsheet is
            sheetData = readtable(fileNames{x}); % Extract the numeric data from the file
            cd(currentFolder) % Return to the original folder
...
if multi_Fo == 0
    for jColumns = 1:width(compiledRaw) % For as many columns are in the table
        if dFF_YN == 0
            fo = mean(tempRaw(start:(start+time2frame-1),jColumns)); % The Fo is the mean of the baseline frames
